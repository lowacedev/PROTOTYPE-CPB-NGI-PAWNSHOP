<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Redeem Transaction: ') }} {{ $transaction->pawn_ticket_number }}
        </h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
                        ...
                        <div>
                            ...
                            <div class="space-y-3 text-sm">
                                ...
                                <div class="flex justify-between border-t dark:border-gray-700 pt-3">
                                    <span class="text-gray-500 dark:text-gray-400">Maturity Date:</span>
                                    <span class="font-semibold text-red-600 dark:text-red-400">{{ $transaction->maturity_date->format('M d, Y') }}</span>
                                </div>
                                <div class="flex justify-between border-t dark:border-gray-700 pt-3">
                                ...
                                <div class="flex justify-between">
                                    <span class="text-gray-500 dark:text-gray-400">Interest Due:</span>
                                    <span class="font-semibold">₱{{ number_format($interestDue, 2) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-blue-50 dark:bg-gray-900 border border-blue-200 dark:border-gray-700 rounded-lg p-6">
                            ...
                            <form method="POST" action="{{ route('transactions.redeem.process', $transaction) }}">
                                @csrf
                                <div class="mb-4">
                                    <label for="amount_paid" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Amount Paid (₱)</label>
                                    <input type="number" name="amount_paid" id="amount_paid" step="0.01" min="{{ $totalDue }}" value="{{ old('amount_paid', $totalDue) }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-bold text-lg" required>
                                    @error('amount_paid')
                                ...
                                <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                                    ...
                                    <button type="submit" class="inline-flex justify-center items-center px-6 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition shadow-md">Confirm Redemption</button>
                                </div>
                            </form>
                        </div>
                    </div>
